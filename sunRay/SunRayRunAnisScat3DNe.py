...
def runRays3d(steps_N  = -1 , collect_N = 180, t_param = 20.0, photon_N = 10000,
            start_r = 1.75, start_theta = 0/180.0*np.pi,    start_phi  = 0/180.0*np.pi,
            f_ratio  = 1.1, ne_r_vec = dm.dens3dcoronalloop,  ne_r_base = dm.parkerfit,
            epsilon = 0.4, anis = 0.2, freq0 = 150.0e6,
    ...
    ne_0_num = ne_r_vec(r_vec_0)

    nu_e0 = 2.91e-6*ne_0_num*20./Te**1.5
    nu_e = nu_e0.to(dev_u)
    ...
    omega0 = freq0*(2*PI)
    nu_s0 = scat.nuScattering(rr,omega0,epsilon,ne_r_base,dev_u=dev_u)

    #if Show_param:
    #    SP.showParameters(ne_r,omega0,epsilon)  

    # wave-vector of the photons
    kc0 = torch.sqrt(omega0**2. - pfreq.omega_pe_r_vec(ne_r_vec,r_vec,dev_u=dev_u)**2.)
    if sphere_gen:
    ...
    domega_pe_dxyz = pfreq.domega_dxyz_vec(ne_r_vec,r_vec.detach(),dev_u=dev_u)
    ...
    if steps_N == -1:
        ...
        if verb_out:
            ...
            print("Absorb Col    : "+str(dt_nue0.cpu().numpy()[0]))
    ...
    #for idx_step in (np.arange(steps_N)):
        ...
        omega = torch.sqrt(pfreq.omega_pe_r_vec(ne_r_vec,r_vec,dev_u=dev_u)**2 + kc_cur**2)
        freq_pe = omega/(2*PI)

        nu_s = scat.nuScattering(rr_cur,omega,epsilon,ne_r_base,dev_u=dev_u)
        ...
        with torch.no_grad(): # no autograd in following calc
            ...
            dk_xyz_dt = ((pfreq.omega_pe_r_vec(ne_r_vec,r_vec,dev_u=dev_u)/omega).repeat(3,1)   
                        * domega_pe_dxyz) * c_r
            ...
            kc_refresh = (torch.sqrt(omega**2-pfreq.omega_pe_r_vec(ne_r_vec,r_vec,dev_u=dev_u)**2)
                /torch.sqrt(torch.sum(k_vec.pow(2),axis=0)))
            k_vec = k_vec * kc_refresh.repeat(3,1)

            nu_e = (2.91e-6 * ne_r_vec(r_vec) * 20. / Te**1.5
                *pfreq.omega_pe_r(ne_r_vec, r_vec, dev_u=dev_u)**2/omega**2)
        ...
        if idx_step in collectPoints:
            ...
            if verb_out==2: # print out the process
                print('F_pe:'+'{:.3f}'.format(np.mean(
                    (pfreq.omega_pe_r_vec(ne_r_vec, torch.mean(r_vec,2),dev_u=dev_u)/2/PI/1e6).cpu().data.numpy()))+
                    ' |  R:'+'{:.3f}'.format(torch.mean(rr_cur).cpu().data.numpy())+
                    ' |  Ne_r:'+'{:.3f}'.format(ne_r_vec(torch.mean(r_vec,2)).cpu().data.numpy())+
